...
| **Audit** | Schema-versioned JSONL records (`schema_version: 2`), hash-chained and ed25519-signed; no raw secrets — the shipped payload digests are `request_digest` / `response_digest` ([record format](spec.md)) |

### Out of scope (v1)

- Layer 2 governance (LLM-side guardian, approval UX, policy authoring)
- Multi-agent orchestration, dashboards, SaaS hosting
- Public transparency logs, timestamping authorities, or inclusion proofs independent of possessing the record file. Schema v2 **is** a hash-chained, ed25519-signed JSONL file; truncation of an unanchored tail is still undetectable from the chain alone (see [record format](spec.md) and [ADR-0002](adr/0002-verify-reports-coverage-not-pass-fail.md))
...
6. **Audit on every exit.** Denials, traps, resource caps, and panics produce
   ...
   **deferred** (not shipped). Wire contract: [`spec/SPEC.md`](spec.md).
...
| [Record format spec](spec.md) | Field-level line contract, chain rule, verdict model, and the non-guarantees stated plainly |
| [Hot-path benchmarks](benchmarks.md) | Policy ≪ 100 µs; combined pipeline ≪ 1 ms (cited hardware) |

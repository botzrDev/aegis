...
> shipped behaviour. `v0.3.0` and current `main` have no `ChainSink` trait: the
> sink is a hard-wired `BufWriter<File>` (`audit/src/writer.rs:36-44`), the fsync
> is unconditional (`:218`), and `Runtime::default()` still uses the temp-file
> sink. Read every "becomes" below as the plan of record on 2026-08-14.
